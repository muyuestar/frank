...
//					if (!Tools.checkPID(payActIdNo)) { // 校验身份证
//						JOptionPane.showMessageDialog(null, "付款人证件号码输入有误！", "提示", JOptionPane.DEFAULT_OPTION);
//						payActIdNoJT.requestFocus();
//						return;
//					}
//				}
...
//					if (!Tools.checkPID(agentIdNo)) { // 校验身份证
//						JOptionPane.showMessageDialog(null, "代理人证件号码输入有误！", "提示", JOptionPane.DEFAULT_OPTION);
//						agentIdNoJT.requestFocus();
//						return;
//					}
//				}
...
//						return;
//					}
//					if (payActIdType.length() > 8) { // 付款人证件种类8
//						JOptionPane.showMessageDialog(null, "付款人证件种类超长！", "提示", JOptionPane.DEFAULT_OPTION);
//						payActIdTypeJT.requestFocus();
//						return;
//					}
//					if (agentIdType.length() > 8) { // 代理人证件种类
//						JOptionPane.showMessageDialog(null, "代理人证件种类超长！", "提示", JOptionPane.DEFAULT_OPTION);
//						agentIdTypeJT.requestFocus();
//						return;
//					}
//					if (recAct.length() > 28) { // 收款人账号28
...
//						return;
//					}
//					if (payActBank.length() > 16) { // 付款人开户行16
//						JOptionPane.showMessageDialog(null, "付款人开户行超长！", "提示", JOptionPane.DEFAULT_OPTION);
//						payActBankJT.requestFocus();
//						return;
//					}
...
//						return;
//					}
//					if (revActBank.length() > 16) { // 收款人开户行16
//						JOptionPane.showMessageDialog(null, "收款人开户行超长！", "提示", JOptionPane.DEFAULT_OPTION);
//						revActBankJT.requestFocus();
//						return;
//					}
//					try {
//						if (tspjName.getBytes("gbk").length > 26) { // 托收凭据名称13汉字，26字节
...
//		dyxzJL.setFont(new Font("宋体", Font.BOLD, 18));
//		dyxzJL.setBounds(dyx, dyy, 95, 22);
//		panel.add(dyxzJL);
//
//		JLabel firstJL = new JLabel("1、打印现金支票、转账支票请撕去存根后再打印；");
//		firstJL.setFont(new Font("宋体", Font.PLAIN, 15));
//		firstJL.setBounds(dyx, dyy + Constants.DYXZY, 380, 22);
//		panel.add(firstJL);
//
//		JLabel secondJL = new JLabel("2、客户凭证打印类型：现金支票、转账支票+进账单、单位业务委托书、个人业务委托书、进账单、现金解款单、托收凭证；");
//		secondJL.setFont(new Font("宋体", Font.PLAIN, 15));
//		secondJL.setBounds(dyx, dyy + 2 * Constants.DYXZY, 850, 22);
//		panel.add(secondJL);
//
//		JLabel thirdJL = new JLabel("3、单位业务委托书为新版业务委托书，不支持旧版业务委托书；");
//		thirdJL.setFont(new Font("宋体", Font.PLAIN, 15));
//		thirdJL.setBounds(dyx, dyy + 3 * Constants.DYXZY, 450, 22);
//		panel.add(thirdJL);
//
//		JLabel fourthJL = new JLabel("4、字符数的控制：按凭证实际可打印情况对字符数进行相应控制，超长情况下程序会提示。");
//		fourthJL.setFont(new Font("宋体", Font.PLAIN, 15));
